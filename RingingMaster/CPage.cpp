...
CPage::CPage(RECT rectDraw, CDC* pDC, int nMapMode)
{
	...
	m_PrtDesc._offset = CSize(0,0);
	m_PrtDesc._doPrintClip = true;
...
/*	This was causing problems with print offset
...
{
	int numColors = m_PrtDesc.pDC->GetDeviceCaps(NUMCOLORS);
//	TRACE("%d colors on the device\r\n", numColors); //8 at home
	if ((numColors >=0)&&(numColors <=2))
	{
		//we have a mono printer - set to black if not white
...
{
	//sjl given a default value to satisfy level 4 warning
	UINT	OldMode = TRANSPARENT;
	UINT    OldColor = RGB(100,100,100);
	va_list t;
	SaveState();
	va_start(t,fmt);
	vsprintf(Buffer,fmt,t);
...
//	Returns:
///////////////////////////////////////////////////////////////////////////
void CPage::Fill(double top,double left,double bottom,double right,UINT Fillflags)
{
	SaveState();
	if(Fillflags != IGNORE_PARAM)
		m_PrtDesc.uFillFlags=Fillflags;			
...
void CPage::ResetClipRect()
{
	GetDisplayContext()->SelectClipRgn(NULL);
}

void CPage::ExcludeClipRect(double top,double left,double bottom,double right)
{
	ConvertArea(top,left,bottom,right);
	CRect rect((int)left,(int)top,(int)right,(int)bottom);
	GetDisplayContext()->ExcludeClipRect(rect);
}
...
double CPage::Print(TABLEHEADER* TheTable,int row,int col,int PointSize,UINT TextFlags,double offset, double vertOffset, char* fmt,...)
{
	...
	if(TheTable->HeaderOnly==FALSE)
		offset = pTable->InsertItem(buff,row,col,PointSize,TextFlags, offset, vertOffset);
	else
...
//	params:
//	Returns: The new offset inches 
///////////////////////////////////////////////////////////////////////////
double  CPrintTable::InsertItem(LPCSTR Text,int row,int col,int PointSize,UINT TextFlags, double offset, double vertOffset)
{                       
	//sjl given a default value to satisfy level 4 warning
	UINT	OldMode = TRANSPARENT;
	UINT    OldColor = RGB(100,100,100);
	...
	//SJL - I disagree - I have removed  it!!
...
*/	//	determine the bounds of the print rect
	...
	//convert offset to pixels - sjl
	int iOffset = p_Print->ConvertToMappedUnits(offset, HORZRES); 
	//could be negative
	int iVertOffset = p_Print->ConvertToMappedUnits(fabs(vertOffset), VERTRES); 
	if (vertOffset <0)
		iVertOffset = (-iVertOffset);

	m_ps->rc.left=SCol+iOffset+2;
	m_ps->rc.right=ECol-2;
	m_ps->rc.top=Row+2 + iVertOffset;
	...
	int newOffset = p_Print->m_nNextPos - SCol -2;
	double dNewOffset = p_Print->ConvertToInches(newOffset, HORZRES);
	return dNewOffset;
}
...
void CPrintTable::FillColumn(int row,int col)
{
	...
	if ((m_pTable->_leftVLinesLimit == -1)||
		((col >= m_pTable->_leftVLinesLimit)&&
		 (col <= m_pTable->_rightVlinesLimit)&&
		 (row >= m_pTable->_topHLinesLimit)&&
		 (row <= m_pTable->_bottomHlinesLimit)))
	{
		int lineWidth = p_Print->ConvertToMappedUnits(.01,VERTRES);
		p_Print->ThePrinter.DrawRect(&p_Print->m_PrtDesc,lineWidth);
	}
...
double CPage::getLengthInches()
{
	return m_LengthInches;

}

double CPage::getWidthinches()
{
	return m_WidthInches;
}
...
void CPage::setOffset(double vert, double horz)
{
	m_PrtDesc._offset = CSize(ConvertToMappedUnits(horz, HORZRES),
							  ConvertToMappedUnits(vert, VERTRES));

	TRACE("Print offset x:%d, y:%d\r\n", m_PrtDesc._offset.cx, m_PrtDesc._offset.cy);
}

void CPage::bracket(double top, double left, double width, double height, double lineWidth, CString name, UINT flag)
{
	SaveState();

	if(flag != IGNORE_PARAM)
		m_PrtDesc.uPenFlags=flag;
	ConvertArea(top,left,height,width);
	
	lineWidth = ConvertToMappedUnits(lineWidth, HORZRES);
	if (lineWidth < 1)
		lineWidth = 1;

 	m_PrtDesc.rc.top=(int)top;
	m_PrtDesc.rc.bottom=(int)height;
	m_PrtDesc.rc.left=(int)left;
	m_PrtDesc.rc.right=(int)width;

	//this draws the actual bracket
	CPoint point = ThePrinter.drawBracket(&m_PrtDesc,(int)lineWidth);

	//find the height of the text
	int charHeight = ThePrinter.getTextExtent(&m_PrtDesc,"1").cy;

	RestoreState();


	Print(point.y - charHeight, 
		  point.x + ConvertToMappedUnits(0.05, HORZRES),
		  name);
}

void CPage::underline(double top, double left, double length, double pitch, UINT flag)
{
	SaveState();

	if(flag != IGNORE_PARAM)
		m_PrtDesc.uPenFlags=flag;
	ConvertArea(top,left,pitch,length);
	if (pitch < 1)
		pitch = 1;
	...
	int lastLeft = (int)left;
	int lastTop = (int)top;

	for (int i=0;i<length;i+=(int)pitch)
	{
		m_PrtDesc.rc.left = lastLeft;
		lastLeft = (int)left + i;
		m_PrtDesc.rc.right= lastLeft;
		
		m_PrtDesc.rc.top		= lastTop;
		lastTop = (int)top + yOffset;
		m_PrtDesc.rc.bottom		= lastTop;

		ThePrinter.DrawLine(&m_PrtDesc,(int)pitch);
...
double CPage::getTextWidth(CString text)
{
	SaveState();

	CSize size = ThePrinter.getTextExtent(&m_PrtDesc,text);

	RestoreState();

	return ConvertToInches(size.cx, HORZRES);

}

double CPage::getTextHeight(CString text)
{
	SaveState();
	...
	return ConvertToInches(size.cy, VERTRES);
