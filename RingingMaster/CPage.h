...
public:
	...
	virtual void    Fill(double top,double left,double bottom,double right,UINT Fillflags=FILL_NONE);
	...
	virtual	double Print(TABLEHEADER* TheTable,int row,int col,int PointSize,UINT TextFlags,double offset, double vertOffset, char* fmt,...);
	...
	virtual void underline(double top, double left, double length, double pitch, UINT flag=PEN_SOLID);
	virtual void bracket(double top, double left, double width, double height, double lineWidth, CString name, UINT flag=PEN_SOLID);
...
public:
	...
	void setOffset(double vert, double horz);
...
public:
		...
		COLUMNDATA	 	ColDesc[500]; // see above //sjl I changed this from 25 to 500
		CPrintTable*	pClsTable;	// ptr to object ownibg this header

		//SJL - added member values
		int				_leftVLinesLimit;
		int				_rightVlinesLimit;
		int				_topHLinesLimit;
		int				_bottomHlinesLimit;
...
protected:
		...
		virtual	double  InsertItem(LPCSTR Text,int row,int col,int PS,UINT Flags, double offset = 0.0 , double vertOffset = 0.0);
