...
protected:
	FlatHeaderCtrl		m_wndFlatHeader;
...
// Operations
public:
	virtual CString getPressedTestString(int row, int column);
	void selectIndex(int index);
	void setUsePressedString(bool usePressedString);
	void endColDrag();
	...
	virtual void AutoSaveColumns(
		// registry section name, if NULL the default is RingingMasterApp::_RegistryBase .
...
protected:
	...
	bool _usePressedString;

	virtual void notifyOrderArray(int iCount, LPINT piArray);
	...
	afx_msg LRESULT OnColumnDragEnd(UINT wParam, LONG lParam);
...
inline DWORD CCJListCtrl::GetExtendedStyle()
	{ ASSERT(::IsWindow(m_hWnd)); return SendMessage(LVM_GETEXTENDEDLISTVIEWSTYLE, 0, 0); }

inline void CCJListCtrl::SetExtendedStyle(DWORD dwExStyle)
	{ ASSERT(::IsWindow(m_hWnd)); SendMessage(LVM_SETEXTENDEDLISTVIEWSTYLE, 0, GetExtendedStyle()|dwExStyle); }

inline void CCJListCtrl::SetRowColors(COLORREF clrText, COLORREF clrTextBk)
	{ m_clrText = clrText; m_clrTextBk = clrTextBk; }

inline void CCJListCtrl::SetMinimumColSize(int nSize)
	{ m_nMinSize = nSize; }

inline void CCJListCtrl::DisableColumnSizing(bool bDisable)
