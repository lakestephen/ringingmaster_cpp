...
BOOL CSplashScreenEx::_bShowSplashWnd;
CSplashScreenEx* CSplashScreenEx::_pSplashWnd;
...
BOOL CSplashScreenEx::Create(CWnd *pWndParent,LPCTSTR szText,DWORD dwTimeout,DWORD dwStyle)
{
	...
	if (!CreateEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,"SplashScreenExClass",NULL,WS_POPUP,0,0,0,0,pWndParent ? pWndParent->m_hWnd : NULL,NULL))
		return FALSE;
...
//defined here as WINVER is less than the required 500. we already protect using the m_fnAnimateWindow.
#define AW_BLEND                    0x00080000
#define AW_HIDE                     0x00010000
#define AW_ACTIVATE                 0x00020000
#define TIMER_ID					0x01234000
...
void CSplashScreenEx::Show()
{
	...
	if (m_dwTimeout!=0)
		SetTimer(TIMER_ID,m_dwTimeout,NULL);
}
...
void CSplashScreenEx::EnableSplashScreen(BOOL bEnable)
{
	_bShowSplashWnd = bEnable;

#ifdef _DEBUG
	_bShowSplashWnd = FALSE; 
#endif
}


void CSplashScreenEx::HideSplashScreen()
{
	if (_pSplashWnd != NULL)
		_pSplashWnd->Hide();
}

void CSplashScreenEx::SetTextStatic(LPCTSTR szText)
{
	if (_pSplashWnd != NULL)
		_pSplashWnd->SetText(szText);
}

void CSplashScreenEx::ShowSplashScreen(CWnd* pParentWnd )
{
	if (!_bShowSplashWnd || _pSplashWnd != NULL)
		return;

	// Allocate a new splash screen, and create the window.
	_pSplashWnd=new CSplashScreenEx();													
	if (!_pSplashWnd->Create(pParentWnd,"",60000,CSS_FADE | CSS_HIDEONCLICK	|
	...
	{
		_pSplashWnd->SetBitmap(IDB_SPLASH,23,255,255);
		_pSplashWnd->SetTextFont("Arial",100,CSS_TEXT_BOLD);
		CRect textRect = CRect(160,365,410,30);
		_pSplashWnd->SetTextRect(textRect );
		_pSplashWnd->SetTextColor(RGB(255,255,255));
		_pSplashWnd->SetTextFormat(DT_SINGLELINE | DT_CENTER | DT_VCENTER);
		_pSplashWnd->Show();
	}
...
void CSplashScreenEx::OnLButtonDown(UINT nFlags, CPoint point)
{
