// TouchAnalysisCourseHead.cpp : implementation file
//
...
TouchAnalysisCourseHead::TouchAnalysisCourseHead()
{
}
...
BEGIN_MESSAGE_MAP(TouchAnalysisCourseHead, ScrollWnd)
	//{{AFX_MSG_MAP(TouchAnalysisCourseHead)
	ON_WM_CONTEXTMENU()
...
/////////////////////////////////////////////////////////////////////////////
// TouchAnalysisCourseHead message handlers

void TouchAnalysisCourseHead::mainFrame_notifyUpdate(ViewType viewType)
{
 ...
 /* todo
	...
	{	 
		...
		if (method) 
		{
			...
			scrollSize.cx *= method->getCourseHeads().GetWidth();
			scrollSize.cy *= (method->getCourseHeads().GetHeight()+2);
			scrollSize.cx += 10;
			scrollSize.cy += 10;
...
BOOL TouchAnalysisCourseHead::Create(CWnd * pParentWnd)
{
...
BOOL TouchAnalysisCourseHead::DestroyWindow()
{
...
void TouchAnalysisCourseHead::OnDraw(CDC* pDC)
{
	TouchDoc * pTouchDoc = mainFrame()->getActiveTouchDoc();
   ...
   /*    todo
	//title;
	for (int i=0;i<method->getCourseHeads().GetWidth();i++)
	{
	...
	for (int j=0;j<method->getCourseHeads().GetHeight();j++)
	{
		...
		for (int i=0;i<method->getCourseHeads().GetWidth();i++)
		{
			if (method->getCourseHeads().GetAt(i,j))
			{
				pDC->TextOut(left,top,method->getCourseHeads().GetAt(i,j)->getAsText());
			}
			left += size.cx;
		}
		top += size.cy ;
...
void TouchAnalysisCourseHead::OnContextMenu(CWnd* /*pWnd*/, CPoint point) 
{
	TouchDoc* pTouchDoc = mainFrame()->getActiveTouchDoc();
	if (pTouchDoc)		
	{
		...
		pTouchDoc->getPrintWhatAnalysis()._bCourse = TRUE;
...
void TouchAnalysisCourseHead::OnUpdatePopupPrintAnalysis(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(mainFrame()->getActiveTouchDoc() != NULL);
	
}

void TouchAnalysisCourseHead::OnPopupPrintAnalysis() 
{
	TouchDoc* pTouchDoc = mainFrame()->getActiveTouchDoc();
...
void TouchAnalysisCourseHead::OnUpdatePrintAnalysis(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(mainFrame()->getActiveTouchDoc() != NULL);
	
}

void TouchAnalysisCourseHead::OnPrintAnalysis() 
{
	TouchDoc* pTouchDoc = mainFrame()->getActiveTouchDoc();
...
void TouchAnalysisCourseHead::OnPopupEditCopy() 
{
	TouchAnalysisBase::OnPopupEditCopy(this);	
}

CString TouchAnalysisCourseHead::getCopyData(const Method& method, const MethodAnalysis& analysis)
{
	CString tempStr;
	CString str;
/* todo	
	...
	for (int j=0;j<method->getCourseHeads().GetHeight();j++)
	{
		for (int i=0;i<method->getCourseHeads().GetWidth();i++)
		{
			if (method->getCourseHeads().GetAt(i,j))
			{
				str += method->getCourseHeads().GetAt(i,j)->getAsText();
...
double TouchAnalysisCourseHead::print(CPage *page, double nextRow)
{
/* todo	nextRow = PrintUtils::printTitle(page, "Course Heads", nextRow, false);
	...
	if (_method) 
	{
		...
		for (int i=0;i<_method->getCourseHeads().GetWidth();i++)
		{
			...
			for (int j=0;j<_method->getCourseHeads().GetHeight();j++)			
			{
				if (_method->getCourseHeads().GetAt(i,j))
				{
					str = _method->getCourseHeads().GetAt(i,j)->getAsText();
