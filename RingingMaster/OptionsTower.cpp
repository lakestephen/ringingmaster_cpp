// OptionsTower.cpp : implementation file
//
...
OptionsTower::OptionsTower()
	: CBCGPPropertyPage(OptionsTower::IDD, NULL)
{
	//{{AFX_DATA_INIT(OptionsTower)
	//}}AFX_DATA_INIT
}


void OptionsTower::DoDataExchange(CDataExchange* pDX)
{
	CBCGPPropertyPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(OptionsTower)
...
BEGIN_MESSAGE_MAP(OptionsTower, CBCGPPropertyPage)
	//{{AFX_MSG_MAP(OptionsTower)
...
/////////////////////////////////////////////////////////////////////////////
// OptionsTower message handlers

/////////////////////////////////////////////////////////////////////////////
// OptionsTower message handlers


BOOL OptionsTower::OnInitDialog() 
{
...
void OptionsTower::update()
{
	...
	const AvailableFields& selected = recordManager()->getSelectedFields(vt_tower);
...
BOOL OptionsTower::isOnSelectedList(AvailableField availableField)
{  
	/* todo 	const AvailableFields& selected = recordManager()->getSelectedFields(vt_tower);
...
BOOL OptionsTower::isOnAvailableList(AvailableField selectedField)
{  
/* todo	const AvailableFields& available = recordManager()->getAvailableFields(vt_tower);
...
void OptionsTower::OnAllLeft() 
{
	/* todo 	AvailableFields& selected = recordManager()->getSelectedFields(vt_tower);
	selected.RemoveAll();	
	recordManager()->resetAllTowerListColumnWidths();
	update();	*/
}

void OptionsTower::OnAllRight() 
{
	/* todo 	const AvailableFields& available = recordManager()->getAvailableFields(vt_tower);
	AvailableFields& selected = recordManager()->getSelectedFields(vt_tower);
	AvailableFields tempList;

	//first add to the temp list
	for (int i=0;i<available.GetSize();i++)
	{				
		if (!isOnSelectedList(available.GetAt(i)))
		{
...
void OptionsTower::OnUpdateAllLeft(CCmdUI* pCmdUI)  
{
	pCmdUI->Enable(_rightCtrl.GetCount() != 0);
}

void OptionsTower::OnUpdateAllRight(CCmdUI* pCmdUI)  
{
	pCmdUI->Enable(_leftCtrl.GetCount() != 0);
}

void OptionsTower::OnUpdateLeft(CCmdUI* pCmdUI)  
{
	pCmdUI->Enable(_rightCtrl.GetSelCount() != 0);
}

void OptionsTower::OnUpdateRight(CCmdUI* pCmdUI)  
{
	pCmdUI->Enable(_leftCtrl.GetSelCount() != 0);
}

void OptionsTower::OnUpdateUp(CCmdUI* pCmdUI)  
{
	int count = _rightCtrl.GetSelCount();
	BOOL enable = FALSE;
...
void OptionsTower::OnUpdateDown(CCmdUI* pCmdUI)  
{
	int count = _rightCtrl.GetSelCount();
	BOOL enable = FALSE;
...
void OptionsTower::selectAvailableFields(AvailableField field)
{
...
void OptionsTower::onReset() 
{
	if(AfxMessageBox("Are you sure you want to reset the fields list?", MB_ICONQUESTION|MB_YESNO) == IDYES)
	{
		//todo recordManager()->setupSelectedTowerFieldsLists();
		//todo recordManager()->resetAllTowerListColumnWidths();
		update();
	}	
}

BOOL OptionsTower::OnKillActive()
{
	/* todo 	if (CBCGPPropertyPage::OnKillActive())
	{
		const AvailableFields& selected = recordManager()->getSelectedFields(vt_tower);
...
BOOL OptionsTower::OnSetActive()
{
	if (!userManager()->isValid())
	{
		AfxMessageBox("Please add or select a user before editing the tower database options. ", MB_ICONHAND);
