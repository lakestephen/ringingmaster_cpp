
#pragma once


#include "ScrollWnd.h"
#include "TouchAnalysisBase.h"



class AllTheWorkBell;
class CPage;
class MethodAnalysis;

/////////////////////////////////////////////////////////////////////////////
// TouchAnalysisSpliceTransition window

class TouchAnalysisSpliceTransition : public ScrollWnd, TouchAnalysisBase  
{
// Construction
public:
	TouchAnalysisSpliceTransition();

// Attributes
public:

// Operations
public:

// Overrides
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(TouchAnalysisSpliceTransition)	
	public:
	virtual BOOL Create(CWnd * pParentWnd);

	//}}AFX_VIRTUAL

// Implementation
public:
	CString getCopyData(const Method& method, const MethodAnalysis& analysis);
	void mainFrame_notifyUpdate(ViewType viewType);
	virtual ~TouchAnalysisSpliceTransition();

	void OnDraw(CDC* pDC);
	static double print(CPage *page, double nextRow);	 

	// Generated message map functions
protected:

	void calculateSizes(CDC* pDC);

	int _gridWidth;
	int _gridHeight;

	//{{AFX_MSG(TouchAnalysisSpliceTransition)
	afx_msg void OnContextMenu(CWnd* pWnd, CPoint point);
	afx_msg void OnUpdatePopupPrintAnalysis(CCmdUI* pCmdUI);
	afx_msg void OnPopupPrintAnalysis();
	afx_msg void OnUpdatePrintAnalysis(CCmdUI* pCmdUI);
	afx_msg void OnPrintAnalysis();
	afx_msg void OnPopupEditCopy();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
public:
	virtual BOOL DestroyWindow();
};

