...
typedef	struct tagPrinter{
							...
							int		MarginOffset;
							CSize   _offset;  //used for pagination
							TEXTMETRIC*	tm;
public:
	bool _doPrintClip;
					}PRTTYPE;		
...
class CPrinter{
public:
	CSize getTextExtent(PRTTYPE *ps, CString str);
	void doOffset(RECT& rect, CPoint offset, bool apply);
	void doOffset(PRTTYPE *ps, bool apply);
	CPrinter();
	virtual ~CPrinter();
	...
	virtual CPoint CPrinter::drawBracket(PRTTYPE *ps,int LineSize);
